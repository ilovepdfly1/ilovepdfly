    ...
    <div className="py-16 md:py-24 overflow-x-hidden">
      <div className="container mx-auto px-6">
        <div className="max-w-4xl mx-auto text-center mb-16 scroll-animate" ref={addAnimatedRef}>
          <h1 className="text-4xl md:text-5xl font-extrabold text-gray-800 dark:text-gray-100">ILovePDFLY Blog</h1>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            {filteredPosts.length > 0 ? (
                filteredPosts.map((post, index) => (
                    <div key={index} className="bg-white dark:bg-black border border-gray-200 dark:border-gray-800 rounded-lg shadow-md overflow-hidden group hover:shadow-xl dark:hover:shadow-gray-800/50 transition-all duration-300 flex flex-col md:flex-row scroll-animate border-glow-hover" ref={addAnimatedRef} style={{transitionDelay: `${index * 50}ms`}}>
                        <Link to={`/blog/${post.slug}`} className="md:w-1/3">
                            <img src={post.image} alt={post.title} className="h-56 w-full object-cover md:h-full" />
                        </Link>
          ...
          <aside className="lg:col-span-1 space-y-8">
            <div className="p-6 bg-white dark:bg-black border border-gray-200 dark:border-gray-800 rounded-lg shadow-md scroll-animate" ref={addAnimatedRef}>
                ...
                {recentPosts.map(post => (
                  <li key={post.slug}>
                    <Link to={`/blog/${post.slug}`} className="group flex items-center gap-4">
                      <img src={post.image} alt={post.title} className="h-16 w-16 object-cover rounded-md flex-shrink-0" />
                      <div>
